...
class RuleSimulator(UserSimulator):
    ...
    def initialize_warmup_episode(self, warmup_user_goal):
        self.state = {}
        self.state['history_slots'] = {}
        self.state['inform_slots'] = {}
        self.state['request_slots'] = {}
    ...
    def initialize_test_episode(self, test_user_goal):
        self.state = {}
        self.state['history_slots'] = {}
        self.state['inform_slots'] = {}
        self.state['request_slots'] = {}
    ...
    def _sample_action(self):
        ...
        if len(self.goal['inform_slots']) > 0:
            ...
            if 'pricerange' in self.goal['inform_slots'].keys():
                self.state['inform_slots']['pricerange'] = self.goal['inform_slots']['pricerange']
                
            for slot in self.goal['inform_slots'].keys():
                if known_slot == slot or slot == 'pricerange': continue
    ...
    def corrupt(self, user_action):
        ...
        self.intent_err_probability = 0.5
    ...
    def response_thanks(self, system_action):
        ...
        if len(request_slot_set) > 0 or len(rest_slot_set) > 0:
            # print ("Failed: request set or rest set not empty")

            # print("Request set: ")
            # pprint(request_slot_set)

            # print("Rest slot set")
            # pprint(rest_slot_set)
        ...
        for info_slot in self.state['history_slots'].keys():
            if self.state['history_slots'][info_slot] == dialog_config.NO_VALUE_MATCH:
                # print ("Failed: not match for a history slot: %s".format(info_slot))
                # sleep(2)
            ...
            if info_slot in self.goal['inform_slots'].keys():
                if self.state['history_slots'][info_slot] != self.goal['inform_slots'][info_slot]:
                    # print ("Mismatch between history slot and a goal slot")
                    # print (info_slot)
                    # sleep(2)
        ...
        if 'ticket' in system_action['inform_slots'].keys():
            if system_action['inform_slots']['ticket'] == dialog_config.NO_VALUE_MATCH:
                # print ("Failed: no ticket slot in the system action")
                # sleep(2)

                self.dialog_status = dialog_config.FAILED_DIALOG
                
        if self.constraint_check == dialog_config.CONSTRAINT_CHECK_FAILURE:
            # print ("Failed: constraint check failure")
            # sleep(2)
    ...
    def response_inform(self, system_action):
        ...
        if 'taskcomplete' in system_action['inform_slots'].keys(): # check all the constraints from agents with user goal
            ...
            for slot in self.goal['inform_slots'].keys():
                ...
                if slot not in system_action['inform_slots'].keys() or (str(self.goal['inform_slots'][slot]).lower() != str(system_action['inform_slots'][slot]).lower()):
