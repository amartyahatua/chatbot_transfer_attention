...
class AgentDQN(Agent):
    def __init__(self, movie_dict=None, act_set=None, slot_set=None, params=None):
        ...
        if params['pretrained_model_path'] != None:
            ...
            print("*******************Pretrained with movie booking and continue training*****************************")
            sleep(5)

            temp_model = copy.deepcopy(self.load_trained_DQN(params['pretrained_model_path']))

            # complitely copy the first layer
            self.dqn.model['Wxh'] = copy.deepcopy(temp_model['Wxh'])
            self.dqn.model['bxh'] = copy.deepcopy(temp_model['bxh'])

            # select only common actions
            #list_to_select = [0, 1, 2, 3, 4, 5, 6, 7, 11, 12, 13]
            list_to_select = [0, 1, 2, 4, 6, 10, 11]

            # hidden-output
            self.dqn.model['Wd'][:, list_to_select] = copy.deepcopy(temp_model['Wd'][:, list_to_select])
            self.dqn.model['bd'][:, list_to_select] = copy.deepcopy(temp_model['bd'][:, list_to_select])
        ...
        if params['trained_model_path'] != None:
            ...
            if params['other_domain'] == 0:
                # print("Pretrained model from the same domain")
                ...
                # print ("Pretrained model from the movie booking")
                temp_model = copy.deepcopy(self.load_trained_DQN(params['trained_model_path']))
    ...
    def initialize_episode(self):
        ...
        self.phase = 0
        self.request_set = ['food', 'city', 'date', 'pricerange', 'numberofpeople']
    ...
    def state_to_action(self, state):
        ...
        self.action = self.run_policy(self.representation, self.testing)
    ...
    def run_policy(self, representation, testing):
        """ epsilon-greedy policy """

        # if we use the agent for testing
        if testing:
