...
if __name__ == "__main__":
    ...
    parser.add_argument('--movie_kb_path', dest='movie_kb_path', type=str, default='./deep_dialog/data/rest_kb.p', help='path to the movie kb .json file')
    ...
    parser.add_argument('--goal_file_path', dest='goal_file_path', type=str, default='./deep_dialog/data/user_goals.p', help='a list of user goals')
    ...
    parser.add_argument('--other_domain', dest='other_domain', type=int, default=0,
                        help='if from other domain')

    parser.add_argument('--pretrained_model_path', dest='pretrained_model_path', type=str, default=None,
                        help='the path for pretrained model in other domain')
    ...
    # parser.add_argument('--training_user_goals_ratio', dest='training_user_goals_ratio', default=1.0, type=float,
    #                     help='Randomly select some percents of the user goals to train')

    parser.add_argument('--testing', dest='testing', default=False, type=bool, help='Testing the agent or not')
...
# load the user goals from .p file and randomly select some proportion of goals
all_goal_set = pickle.load(open("./deep_dialog/data/user_goals_first_turn_template.part.movie.v1.p", 'rb'))

# if params['training_user_goals_ratio'] != 1.0:
#     print ("Use %s of the goals".format(params['training_user_goals_ratio']))
#
#
#     num_elements = int(floor(params['training_user_goals_ratio'] * len(all_goal_set)))
#     random.shuffle(all_goal_set)
#     all_goal_set = all_goal_set[0:num_elements]
...
movie_kb = pickle.load(open("./deep_dialog/data/movie_kb.1k.p", 'rb'))
...
agent_params['pretrained_model_path'] = params['pretrained_model_path']
agent_params['warm_start'] = params['warm_start']
agent_params['cmd_input_mode'] = params['cmd_input_mode']
agent_params['other_domain'] = params['other_domain']
agent_params['testing'] = params['testing']
...
def run_test_episodes(test_user_goals, num_test_repetition):
    print ("Start testing")
    test_performance = {}
    ...
    for num_test in range(num_test_repetition):
        ...
        for test_user_goal in test_user_goals:
            dialog_manager.initialize_test_episode(test_user_goal)
            episode_over = False
            ...
            while (not episode_over):
                ...
                if episode_over:
                    if reward > 0:
                        ...
                        test_cumulative_turns += dialog_manager.state_tracker.turn_count
...
if params['testing']:
    test_performance = run_test_episodes(goal_set['all'], 10)

    print ("Success Rate: {0}".format(test_performance['success_rate']))
    print ("Cumulative Turns: {0}".format(test_performance['ave_turns']))
    print ("Cumulative Reward: {0}".format(test_performance['ave_reward']))
